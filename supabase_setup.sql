-- NetFlow ISP Reseller Billing V15
...
create policy profile_select on public.isp_profiles for select using (
  user_id = auth.uid()
  or exists (
    select 1 from public.isp_profiles me
    where me.user_id = auth.uid()
      and me.workspace_id = isp_profiles.workspace_id
      and me.role in ('owner','admin')
  )
);

drop policy if exists profile_update_self on public.isp_profiles;
create policy profile_update_self on public.isp_profiles for update using (user_id = auth.uid()) with check (user_id = auth.uid());
...
create policy workspace_select_member on public.isp_workspaces for select using (
  user_id = (select workspace_id from public.isp_profiles where user_id = auth.uid())
  and exists (select 1 from public.isp_profiles p where p.user_id = auth.uid() and p.role <> 'unassigned')
);
...
create policy workspace_insert_admin on public.isp_workspaces for insert with check (
  user_id = (select workspace_id from public.isp_profiles where user_id = auth.uid())
  and exists (select 1 from public.isp_profiles p where p.user_id = auth.uid() and p.role in ('owner','admin','manager'))
);

drop policy if exists workspace_update_editor on public.isp_workspaces;
create policy workspace_update_editor on public.isp_workspaces for update using (
  user_id = (select workspace_id from public.isp_profiles where user_id = auth.uid())
  and exists (select 1 from public.isp_profiles p where p.user_id = auth.uid() and p.role in ('owner','admin','manager','billing'))
) with check (user_id = (select workspace_id from public.isp_profiles where user_id = auth.uid()));
...
create policy workspace_delete_owner on public.isp_workspaces for delete using (
  user_id = (select workspace_id from public.isp_profiles where user_id = auth.uid())
  and exists (select 1 from public.isp_profiles p where p.user_id = auth.uid() and p.role = 'owner')
);
...
create policy invite_select_admin on public.isp_staff_invites for select using (
  exists (select 1 from public.isp_profiles p where p.user_id = auth.uid() and p.workspace_id = isp_staff_invites.workspace_id and p.role in ('owner','admin'))
);
